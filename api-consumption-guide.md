...
## 2. Authentication Flow & Token Storage

SignalNG uses Bearer Token authentication via JSON Web Tokens (JWT).
...
On all authenticated requests, pass the JWT token in the `Authorization` header:
```http
Authorization: Bearer <your_jwt_access_token>
```

### Frontend Token Storage Recommendations
- Store access tokens in memory or secure storage (e.g. `SecureStore` on mobile, `httpOnly` secure cookies or secure web storage).
- Clear tokens upon calling `POST /api/auth/logout`.
